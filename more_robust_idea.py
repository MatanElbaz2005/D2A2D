...
ECC_SYMBOLS = 32
SYNC_PRBS_LENGTH = 63  # Robust sync
SYNC_PRBS_POLY = [6, 5]  # x^6 + x^5 + 1
CHIP_LENGTH = 3
DATA_PRBS_POLY = [2, 1]  # x^2 + x + 1, for short chip
...
def encode_udp_to_frame(udp_data: bytes) -> np.ndarray:
    """
    Encode UDP packet (H.265 stream) into a black/white NTSC/PAL frame with spread-spectrum.
    Args:
        udp_data: Bytes (e.g., 32 KB chunk of H.265).
    Returns:
        numpy.ndarray: 720x480 grayscale frame (uint8, 0=black, 255=white).
    """
...
def decode_frame_to_udp(frame: np.ndarray, corr_threshold: float = 0.8) -> bytes:
    """
    Decode a noisy frame back to UDP packet.
    Args:
        frame: numpy.ndarray (720x480, uint8).
        corr_threshold: Min correlation for sync detection.
    Returns:
        bytes: Decoded UDP packet.
    """
    if frame.shape != (FRAME_HEIGHT, FRAME_WIDTH):
        raise ValueError(f"Frame size mismatch: expected {FRAME_HEIGHT}x{FRAME_WIDTH}")
    bit_stream = (frame.flatten() > 127).astype(int)
    received_pm = 2 * bit_stream - 1
    ...
    rx_bits = (np.sign(rx_bits_pm) + 1) / 2
...
if __name__ == "__main__":
    sample_udp_data = b'\x00\x01\x02\x03' * 8192  # 32 KB
    try:
