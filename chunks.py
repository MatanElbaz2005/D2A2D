...
if __name__ == "__main__":
    ...
    while cap.isOpened() and frame_count < 4:
        ...
        try:
            frame_bin = np.zeros((FRAME_HEIGHT, FRAME_WIDTH), dtype=np.uint8)
            for tr in range(TILE_ROWS):
                for tc in range(TILE_COLS):
                    y0, y1 = tr*TILE_H, (tr+1)*TILE_H
                    x0, x1 = tc*TILE_W, (tc+1)*TILE_W
                    roi = frame_proc[y0:y1, x0:x1]
                    enc = encode_frame_to_webp(roi, quality=20)
                    tile_bin = encode_tile(enc) 
                    frame_bin[y0:y1, x0:x1] = tile_bin
            ...
            for tr in range(TILE_ROWS):
                for tc in range(TILE_COLS):
                    y0, y1 = tr*TILE_H, (tr+1)*TILE_H
                    x0, x1 = tc*TILE_W, (tc+1)*TILE_W
                    tile_noisy = noisy[y0:y1, x0:x1]
                    try:
                        dec_bytes = decode_tile(tile_noisy)
                        img = decode_webp_to_frame(dec_bytes) 
                        if img is not None:
                            restored[y0:y1, x0:x1] = img
                    except ValueError:
                        pass
