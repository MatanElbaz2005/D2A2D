...
TILE_ROWS = 5
TILE_COLS = 5
TILE_W    = FRAME_WIDTH  // TILE_COLS   # 720 // 5 = 144
TILE_H    = FRAME_HEIGHT // TILE_ROWS   # 480 // 5 =  96
...
def encode_tile(udp_data: bytes) -> np.ndarray:
    """
    ...
    frame = full_stream.reshape((TILE_H, TILE_W))
    return frame

def decode_tile(frame: np.ndarray, corr_threshold: float = 0.8) -> bytes:
    """
    Decode a noisy frame with vectorized despreading.
    """
    if frame.shape != (TILE_H, TILE_W):
        raise ValueError(f"Tile size mismatch: expected {TILE_H}x{TILE_W}")
...
def encode_frame_tiled(payload: bytes) -> np.ndarray:
    start_encoding_time = time.time()
    chunk_size = (len(payload) + TILE_ROWS*TILE_COLS - 1) // (TILE_ROWS*TILE_COLS)
    ...
    for r in range(TILE_ROWS):
        for c in range(TILE_COLS):
            ...
            idx  += chunk_size
            tile  = encode_tile(chunk)
            y0, y1 = r*TILE_H, (r+1)*TILE_H
            x0, x1 = c*TILE_W, (c+1)*TILE_W
            frame[y0:y1, x0:x1] = tile
    print(f"Encoded {len(payload)} bytes into tiled frame {TILE_ROWS}x{TILE_COLS} in {time.time() - start_encoding_time} sec")
    return frame

def decode_frame_tiled(frame: np.ndarray) -> bytes:
    corrupted_tiles = 0
    ...
    for r in range(TILE_ROWS):
        for c in range(TILE_COLS):
            ...
            try:
                pieces.append(decode_tile(tile))
            except ValueError:
                pieces.append(b'')
                corrupted_tiles += 1
    print(f"Decoded {len(pieces)} tiles in {time.time() - start_decode_time} sec")
...
if __name__ == "__main__":
    ...
    while cap.isOpened() and frame_count < 4:
        ...
        try:
            frame_bin = encode_frame_tiled(full_bytes)
            ...
            noisy ^= noise_mask
            decoded_data = decode_frame_tiled(noisy)
            start_decode_time = time.time()
            if FORMAT_IMAGE == 'jpg':
