...
from sender.transport_tcp import TcpClient
...
HOST = cfg.get("transport", {}).get("connect_host", "127.0.0.1")
...
try:
    while True:
        ok, hdr, payload = client.recv_frame()
        ...
        w = hdr["width"]; h = hdr["height"]; ch = hdr["channels"]

        try:
            if ch == 1:
                chips = np.frombuffer(payload, dtype=np.uint8).reshape(h, w)
            elif ch == 3:
